...
class CreateClient extends TelegramClient {
  constructor(stringSession, apiId, apiHash, options) {
    ...
    this.addEventHandler((event)=>{
      ...
      const type = message.quoted?`${message.jid}-quoted-${message.quoted}`:`${message.jid}-message`
    ...
    try {
      if (obj.text) {
        ...
        if (obj.document.url) {
          ...
          await this.sendFile(id, { file: result, forceDocument: true,workers:10,...options });
        } else {
          const result = new CustomFile(
            obj.fileName,
            obj.document.length,
